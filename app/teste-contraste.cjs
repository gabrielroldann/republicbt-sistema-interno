...
(async () => {
  ...
  await pg.goto('http://localhost:4501/painel', { waitUntil: 'networkidle' });
  ...
  for (const tema of ['navy', 'dark', 'light']) {
    // A chave do painel é `republicbt:tema`; a do CRM é `republicbt:crm:tema`.
    // Cada app guarda o seu, de propósito: quem prefere claro no CRM pode
    // preferir escuro no painel. Usar a chave errada faz os três temas medirem
    // a mesma cor — e o teste passa sem exercitar nada.
    //
    ...
    const medido = await pg.evaluate(() => {
      // A ação primária do painel é um NavLink (leva para /vendas/nova), não
      // um <button>. Procurar só por `button` não acharia nada — e o teste
      // falharia por não achar, não por contraste.
      const b = document.querySelector('aside a[href="/painel/vendas/nova"]');
      if (!b) return null;
